...
**Docker GPU access (ROCm profile):** Pawpile’s `inference-rocm` service already mounts `/dev/kfd` and `/dev/dri`, adds the `video`/`render` groups, and relaxes seccomp so HIP can open the AMD devices. You do **not** need `--gpus all` (that is NVIDIA-specific). On the host, confirm `rocm-smi` lists both cards before starting Docker. If `rocm-smi` only reports generic names, Pawpile labels cards as **AMD GPU 0 (32 GB)** / **AMD GPU 1 (32 GB)** with a short PCI slot ID (e.g. **69:00.0**) in the subtitle. Refresh the Devices page after updating drivers.
