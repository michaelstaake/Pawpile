...
export default function TermsAndPoliciesPage() {
  ...
  const [isSaving, setIsSaving] = useState<"enabled" | "content" | null>(null);
  ...
  async function updateEnabled(enabled: boolean) {
    ...
    setIsSaving("enabled");
    try {
      const response = await apiPatch<TermsSettings, TermsSettings>("/api/terms/settings", { ...previousSettings, terms_enabled: enabled }, token);
      setLocalSettings(response);
      await refreshAuthState();
      showSuccess("Terms and policies setting updated.");
    } catch (error) {
      setLocalSettings(previousSettings);
      showError(error instanceof Error ? error.message : "Failed to update terms and policies setting");
    } finally {
      setIsSaving(null);
    }
  }
  ...
  async function updateContent(content: string) {
    ...
    const previousSettings = localSettings;
    const nextSettings = { ...previousSettings, terms_content: content };
    setLocalSettings(nextSettings);
    setIsSaving("content");
    try {
      const response = await apiPatch<TermsSettings, TermsSettings>("/api/terms/settings", nextSettings, token);
      setLocalSettings(response);
      await refreshAuthState();
      showSuccess("Terms and policies content updated.");
    } catch (error) {
      setLocalSettings(previousSettings);
      showError(error instanceof Error ? error.message : "Failed to update terms and policies content");
    } finally {
      setIsSaving(null);
    }
  }
